...
class IRrtStar:
    def __init__(self, x_start, x_goal, step_len,
                 ...
                 dominion, state_validity_checker):
    ...
    def planning(self):
      ...
      for k in range(self.iter_max):
          ...
          if not self.svc.is_valid((x_rand.x, x_rand.y)):
            continue 
          x_nearest = self.Nearest(self.V, x_rand)
          x_new = self.Steer(x_nearest, x_rand)

          if x_new and not self.svc.CheckObstacleCollision((x_nearest.x, x_nearest.y), (x_new.x, x_new.y)) and  self.svc.is_valid((x_nearest.x, x_nearest.y)) and self.svc.is_valid((x_new.x, x_new.y)):
              ...
              if self.InGoalRegion(x_new):
                  if not self.svc.CheckObstacleCollision((x_new.x, x_new.y), (self.x_goal.x, self.x_goal.y)):
    ...
    def Near(self, nodelist, node):
        ...
        X_near = [nodelist[ind] for ind in range(len(dist_table)) if dist_table[ind] <= r ** 2 and
                  not self.svc.CheckObstacleCollision((nodelist[ind].x,nodelist[ind].y), (node.x,node.y))]
